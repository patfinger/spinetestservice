...
function getClientCredentialsTokenByTenant(correlationId, tenant, credentials, zoneInfo, cb) {
  const config = {
    credentials: credentials.uaa || credentials,
  ...
  return xssec.requests.requestClientCredentialsToken(tenant, config, null, zoneInfo && zoneInfo.zoneId || null, (err, accessToken, tokenInfo) => {
    if (err) {
      tracer.error('Error getting client_credentials token for ' + credentials.label + ' error' + err.message);
      return cb(err);
    }
    cb(null, {
      accessToken: tokenInfo.access_token,
      tokenType: tokenInfo.token_type,
      tokenRefreshTimestamp: passportUtils.getExpiresAt(tokenInfo.expires_in).getTime()
    });
  });
}
...
function getClientCredentialsToken(subdomain, correlationId, credentials, zoneId) {
  return new Promise((resolve, reject) => {
    ...
    xssec.requests.requestClientCredentialsToken(subdomain, config, null, zoneId, (err, token, tokenJson) => {
      if (err) {
        return reject('Failed to create client_credentials token' + err);
      }
      resolve({
        accessToken: tokenJson.access_token,
        tokenType: tokenJson.token_type,
        tokenRefreshTimestamp: passportUtils.getExpiresAt(tokenJson.expires_in).getTime()
      });
    });
  });
}
